...
| Mechanics | `Fis_runtime/mechanics.js` | операторы, арность, векторные компоненты, индексы связности |
...
| Geo ontology | `Geo_style/geo_core.json`, `geo_ops.json` | сорты Point/Vec2/Frame…, конструкторы, ops |

**Порядок подключения (projections.html):**
`units → mechanics → presentation → projection → package → geo_compute → construct_layout`

**Mechanics:** occurrence = quantity + role + operator/arity + indexes. Индексы не являются величинами: они адресуют occurrence по событию, принадлежности конструкции и координате.
