...
(function (global) {
  ...
  function matchCondition(item, cond) {
    ...
    if (Array.isArray(cond.math_kind_in)) {
      if (cond.math_kind_in.indexOf(String(item.math_kind || "")) < 0) return false;
    }
  ...
  function collectCriterionValues(data, lang, criterion, pool) {
    ...
    (pool || []).forEach(function (q) {
      if (collect === "domains") {
        ...
        set.set(String(q.invariance), String(q.invariance));
  ...
  function matchesFilters(data, item, filters, cardType) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (key === "domain" || collect === "domains") {
        ...
        if (String(item.invariance || "") !== val) return false;
      ...
      list_items: function (ctx) {
        ...
        if (q) {
          rows = rows.filter(function (item) {
            ...
            const usages = data.usages && data.usages.usages && data.usages.usages[item.id];
            let blob = [item.id, item.notes, item.dimension, item.category]
              .join(" ")
              .toLowerCase();
            (usages || []).forEach(function (u) {
              blob +=
