...
(function (global) {
  ...
  function mountLawGraphs(root, data) {
    if (!root || !window.GeoCompute) return;
    const hosts = root.querySelectorAll("[data-law-graph]");
    if (!hosts.length) return;

    const structures = getStructuresList(data && data.structures);
    const domain = [0, 4];
    ...
    hosts.forEach(function (host) {
      ...
      if (!struct || !struct.ast) {
        showMsg("Структура " + structureRef + " не найдена");
        return;
      }
