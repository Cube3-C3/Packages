...
(function (global) {
  ...
  function astToDisplay(node, ctx, parentPrec, options) {
    ...
    function primarySymbol(qid) {
      const list = usagesData && usagesData.usages && usagesData.usages[qid];
      if (Array.isArray(list) && list[0] && list[0].symbol != null) return String(list[0].symbol);
      return qid || "?";
    }
    ...
    function isAtomicText(t) {
      ...
      if (/^[^<>+\−·/=^]{1,6}$/.test(plain) && plain.indexOf(" ") < 0) return true;
    ...
    function collectMulFactors(n, inverted) {
      ...
      if (n && typeof n === "object" && n.op === "div") {
        ...
        if (args.length >= 2 && isNeutralOne(args[0]) && !isEmptyNode(args[1])) {
          // (1/x)^k with inverted flips
      ...
      if (n && typeof n === "object" && n.ref) {
        const s = emitSym(n.ref, esc(primarySymbol(n.ref)));
        return [{ html: s, isNum: false, isOne: false, isDiv: inverted }];
      }
    ...
    function render(node, parentP) {
      ...
      if (node.ref) return emitSym(node.ref, esc(primarySymbol(node.ref)));
  ...
  function bindingToLeaf(binding) {
    ...
    if (typeof binding === "object") {
      ...
      if (binding.ref) return { ref: binding.ref };
      if (binding.quantity_id) return { ref: binding.quantity_id };
    }
...
(function () {
  ...
  /**
   * ПРАВИЛО ПРОЕКТА (зафиксировано):
   * Первичный символ величины = usages[qid][0].symbol
   * — всегда первый элемент массива, без сортировки по role/domain.
   * Используется во всех проекциях формул и кратких отображениях.
   */
  function primarySymbol(qid, usagesData) {
    const list = usagesData?.usages?.[qid];
    if (Array.isArray(list) && list.length && list[0].symbol != null) {
      return String(list[0].symbol);
    }
    return qid; // fallback, если usages ещё не загружены
  }
  ...
  function buildPresentationCtx(ctx) {
    ...
    if (q) {
      ...
      if (!derived.usages_rows) {
        derived.usages_rows = (usages || []).map((u) => {
      ...
      if (!derived.related_formulas && qid) {
        const related = formulasUsing(data.formulas, qid, data.structures);
  ...
  function renderFormulaStub(container, projection, state, data) {
    ...
    // график всегда (даже если в загруженной ontology ещё нет law_graph)
    if (html.indexOf('data-law-graph="1"') < 0) {
      const structureRef = (lawForPres && lawForPres.structure_ref) || (inst && inst.structure_ref) || "";
      const graphLabel = lang === "en" ? "Graph" : "График";
      html +=
        '<div class="section law-graph-slot">' +
          '<div class="card law-graph-card">' +
            '<div class="label">' + graphLabel + "</div>" +
            '<div class="law-graph-host" data-law-graph="1"' +
              ' data-structure-ref="' + escapeHtml(structureRef) + '"' +
              ' data-law-id="' + escapeHtml(String(lawId)) + '">' +
              '<canvas class="law-graph-canvas" width="320" height="200"></canvas>' +
              '<div class="law-graph-msg pres-muted" hidden></div>' +
            "</div>" +
          "</div>" +
        "</div>";
    }
    html += `</div>`;
    container.innerHTML = html;
    // график после вставки DOM (нужен canvas + GeoCompute)
    try {
      if (window.FisPresentation && typeof window.FisPresentation.mountLawGraphs === "function") {
        window.FisPresentation.mountLawGraphs(container, data);
      } else {
        const m = container.querySelector(".law-graph-msg");
        if (m) {
          m.hidden = false;
          m.textContent = "mountLawGraphs недоступен (обнови code.js на GitHub)";
        }
        console.warn("[law-graph] FisPresentation.mountLawGraphs missing");
      }
    } catch (e) {
      console.error("[law-graph] mount failed", e);
      const m = container.querySelector(".law-graph-msg");
      if (m) {
        m.hidden = false;
...
(function (global) {
  ...
  /**
   * Слот графика закономерности (простой: оси + кривая, без интерактива).
   * Данные кривой считаются в mountLawGraphs после вставки HTML (нужен GeoCompute).
   */
  function renderLawGraphSlot(ctx, compose) {
    const law = ctx && ctx.law;
    const structureRef = (law && law.structure_ref) || (ctx.inst && ctx.inst.structure_ref) || "";
    const lawId = (law && (law.law_id || law.id)) || "";
    const wrapper = (compose && compose.wrapper) || "law-graph-slot";
    return (
      '<div class="section ' + wrapper + '">' +
        '<div class="card law-graph-card">' +
          '<div class="label">' + (ctx.lang === "en" ? "Graph" : "График") + "</div>" +
          '<div class="law-graph-host" data-law-graph="1"' +
            ' data-structure-ref="' + escapeHtml(structureRef) + '"' +
            ' data-law-id="' + escapeHtml(String(lawId)) + '">' +
            '<canvas class="law-graph-canvas" width="320" height="200"></canvas>' +
  ...
  /**
   * Нарисовать оси + кривую на canvas. coeffs = 1, domain = [0, 4].
   * input = последний операнд, output = O1.
   */
  function mountLawGraphs(root, data) {
    if (!root) return;
    const hosts = root.querySelectorAll("[data-law-graph]");
    try {
      console.info("[law-graph] mount", {
        hosts: hosts.length,
        hasGeo: !!window.GeoCompute,
        structuresType: data && data.structures ? typeof data.structures : "none"
      });
    } catch (e) { /* */ }
    ...
    const structures = listStructures(data && data.structures);
    const domain = [0, 4];

    hosts.forEach(function (host) {
      const canvas = host.querySelector("canvas.law-graph-canvas");
      const msg = host.querySelector(".law-graph-msg");
      const structureRef = host.getAttribute("data-structure-ref") || "";
      if (!canvas) {
        if (msg) {
          msg.hidden = false;
          msg.textContent = "Нет canvas";
        }
        return;
      }

      // явный размер отрисовки (CSS height:auto иногда даёт 0)
      canvas.width = 320;
      canvas.height = 200;
      ...
      if (!window.GeoCompute) {
        showMsg("GeoCompute не загружен");
        return;
      }
      if (typeof window.GeoCompute.curveFromStructure !== "function") {
        showMsg("GeoCompute.curveFromStructure отсутствует — обнови geo_compute.js");
        return;
      }
      if (!structureRef) {
        showMsg("Нет structure_ref");
        return;
      }
      ...
      if (!struct || !struct.ast) {
        showMsg("Структура " + structureRef + " не найдена (structures: " + structures.length + ")");
        return;
      }

      const operandIds = window.GeoCompute.collectOperandIds(struct.ast);
      if (!operandIds || operandIds.length < 2) {
        showMsg("Мало операндов для графика");
        return;
      }
      ...
      let built;
      try {
        built = window.GeoCompute.curveFromStructure(struct, {
          inputOperandId: inputOperandId,
          outputOperandId: outputOperandId,
          domain: domain
        });
      } catch (err) {
        showMsg(String(err && err.message ? err.message : err));
        return;
      }

      const points = built.sample(64);
      if (!points.length) {
        showMsg("Пустая кривая");
        return;
      }

      const finite = points.filter(function (p) {
        return p && isFinite(p.x) && isFinite(p.y);
      });
      if (!finite.length) {
        showMsg("Все точки NaN — проверь eval AST");
        return;
      }

      drawLawGraphCanvas(canvas, finite, domain);
      if (msg) {
        msg.hidden = false;
        msg.style.display = "block";
        msg.textContent =
      ...
      try {
        console.info("[law-graph] drawn", structureRef, finite.length, finite[0], finite[finite.length - 1]);
      } catch (e) { /* */ }
    });
  }

  function drawLawGraphCanvas(canvas, points, domainX) {
    ...
    yMin -= yPad;
    yMax += yPad;
    // если почти всё положительное — прижать низ к 0
    ...
    // оси (светлее — иначе на #171a21 почти не видны)
    ...
    ctx.textAlign = "right";
    ctx.fillText(formatGraphNum(yMin), padL - 4, sy(yMin) + 3);
    ctx.fillText(formatGraphNum(yMax), padL - 4, sy(yMax) + 3);
  ...
  function renderSlot(data, slotId, ctx) {
    ...
    if (mode === "law_graph") return renderLawGraphSlot(ctx, compose);
...
(function (global) {
  ...
  function idFieldForCardType(cardType) {
    const map = { formulas: "law_id", phys_quant: "quantity_id", math_const: "quantity_id" };
  ...
  function matchesFilters(data, item, filters, cardType) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      const criterion = filtersForCardType(data, cardType).find(function (c) {
        return c && c.id === key;
      });
      ...
      if (key === "domain" || collect === "domains") {
        ...
        if (domains.indexOf(val) < 0) return false;
  ...
  function handlers(data) {
    ...
    return {
      ...
      filter_schema: function (ctx) {
        ...
        return filtersForCardType(data, cardType)
      ...
      list_items: function (ctx) {
        ...
        let rows = entitiesForCardType(data, cardType).filter(function (item) {
          return matchesFilters(data, item, filters, cardType);
        });
        ...
        try {
          if (cardType === "formulas") {
            ...
            global.Projection.render(container, {
              ...
              state: { law_id: state.law_id, card_type: cardType }
            });
            return;
          }
          if (!state.quantity_id) {
          ...
          global.Projection.render(container, {
            ...
            state: { quantity_id: state.quantity_id, card_type: cardType }
          });
        } catch (err) {
          container.innerHTML =
            '<div class="empty" style="color:var(--danger);text-align:left">' +
